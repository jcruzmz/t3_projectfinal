...
public class ExceptionControllerAdvice {
	...
	public ResponseEntity<ExceptionMessage> notFound(DataNotFoundException ex, WebRequest request) {
	...
	public ResponseEntity<ExceptionMessage> clientDown(ClientNotFoundException ex, WebRequest request) {
		ExceptionMessage message = new ExceptionMessage("Client is down", 404, request.getDescription(false),
				ex.getMessage());
		return new ResponseEntity<>(message, HttpStatus.NOT_FOUND);
	}
	...
	public ResponseEntity<ExceptionMessage> validationBody(MethodArgumentNotValidException ex, WebRequest request) {
	    ...
	    ExceptionMessage message = new ExceptionMessage("Incorrect body", 400,request.getDescription(false) ,errors.toString());
